...
/** 
 * My variables
 * @var {String} overusedWordCounter - A counter to keep track of how many times overused words appear in the story
 * @var {Number} sentenceCounter - Counter to keep track of how many sentences are in the story
 * @var {Set} checkedWords - Use a Set to keep track of unique words - don't want to check words we've already counted
 * @var {Map} wordCountMap - Use a map to hold the word (key) and number of times it appears (value)
 */
...
/** 
 * Task 3
 * Filter out all the 'unnessary' words:
...
/** 
 * Task 5
 * Count the number of sentences in the Story - all sentences either end with a . or !
...
/** 
 * This function determines which word appears the most in the story
 * Takes the storyWords array and compares it to itself 
 * Populates the wordCountMap which we print
 * TODO - strip out punctuation...
 * @function 
 * @param {Array} wordArray - the Array you want to verify
 */
function mostWords(wordArray){
    for (i=0; i<wordArray.length; i++){
        let instanceCounter = 0;
        let firstWord = wordArray[i].toLowerCase(); // Deal with everything is lowercase...
    
        for (j=0; j<wordArray.length; j++){
            let secondWord = wordArray[j].toLowerCase();
...
// Print the Map of "most words"
// First call the mostWords function, passing in the filtered story array
// This function populates the wordCountMap
mostWords(betterWords);
